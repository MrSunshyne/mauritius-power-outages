...
const latestStatus = ref<'idle' | 'pending' | 'success' | 'error'>('pending') // Start as pending
  ...
  <div class="w-full max-w-4xl mx-auto px-4 py-6 sm:px-6 sm:py-8 flex-1 flex flex-col">
    ...
    <main class="flex flex-col gap-6 sm:gap-8">
      ...
      <div class="hidden sm:flex sm:items-center sm:justify-between">
        ...
        <div class="flex gap-2">
          <button class="btn" :disabled="isLoading" :data-umami-event="ANALYTICS_EVENTS.NAV_PREV" @click="goToPrev">Prev</button>
          <button v-if="!isToday" class="btn" :disabled="isLoading" :data-umami-event="ANALYTICS_EVENTS.NAV_TODAY" @click="goToToday">Today</button>
          <button v-if="!isToday" class="btn" :disabled="isLoading" :data-umami-event="ANALYTICS_EVENTS.NAV_NEXT" @click="goToNext">Next</button>
        </div>
      </div>
    ...
    <nav class="fixed bottom-0 left-0 right-0 sm:hidden mobile-nav">
      <div class="max-w-4xl mx-auto px-4 py-3 flex gap-2">
        <button class="btn-mobile" :disabled="isLoading" :data-umami-event="ANALYTICS_EVENTS.NAV_PREV" @click="goToPrev">
          <span class="text-lg">&larr;</span>
          <span>Prev</span>
        </button>
        <button class="btn-mobile" :disabled="isToday || isLoading" :data-umami-event="ANALYTICS_EVENTS.NAV_TODAY" @click="goToToday">
          <span class="text-lg">&bull;</span>
          <span>Today</span>
        </button>
        <button class="btn-mobile" :disabled="isToday || isLoading" :data-umami-event="ANALYTICS_EVENTS.NAV_NEXT" @click="goToNext">
          <span>Next</span>
