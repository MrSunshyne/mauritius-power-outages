...
const fetchLocality = async () => {
  try {
    ...
    const transformedOutages: Record[] = data.outages.map((outage: any) => ({
      date: outage.date,
      locality: data.name,
...
const title = computed(() => `Power Outages in ${localityName.value} - Mauritius`)
...
defineOgImageComponent('Outage', {
  ...
  description: computed(() => `${localityData.value?.outageCount || 0} outages recorded in this locality`),
  ...
  <div class="w-full max-w-4xl mx-auto px-4 py-6 sm:px-6 sm:py-8 flex-1 flex flex-col">
    <header class="mb-6 sm:mb-8">
      ...
      <p v-if="localityData" class="text-white/50 text-sm mt-1">
        {{ localityData.outageCount }} recorded outage{{ localityData.outageCount !== 1 ? 's' : '' }}
      </p>
    </header>
    ...
    <main class="flex flex-col gap-6 sm:gap-8">
      ...
      <template v-else-if="localityData">
        <div v-if="localityData.outages.length > 0" class="grid gap-4 sm:gap-6">
          <LocalityCell 
