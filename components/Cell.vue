    ...
    >
        <div class="md:flex md:justify-between md:items-center">
            <div class="relative pr-14 sm:pr-16 md:pr-0">
                <div class="mb-1">
                    <div class="text-xs uppercase text-white/70">
                        <ClientOnly>
                            {{ timeUntil }}
                            <template #fallback>
                                <span>{{ formatDate(props.data.from) }}</span>
                            </template>
                        </ClientOnly>
                        <span class="text-white/40"
                            >{{ formatDate(props.data.from) }} -
                            {{ formatDate(props.data.to) }}</span
                        >
                    </div>
                    <NuxtLink
                        :to="`/locality/${getLocalitySlug(props.data)}`"
                        @click.stop
                        class="font-bold text-base sm:text-lg hover:underline"
                        @click="handleLocalityClick"
                    >
                        {{ props.data.locality }}
                    </NuxtLink>
                </div>
                <div
                    class="text-sm max-w-xl text-blue-300 md:py-0 capitalize leading-relaxed"
                >
                    {{ props.data.streets }}
                </div>
            </div>
            <div
                class="md:flex text-sm items-center justify-end mt-3 pt-3 border-t border-white/10 md:mt-0 md:pt-0 md:border-0"
            >
                <ClientOnly>
                    <div class="md:flex flex-col md:items-end">
                        <div class="inline md:block text-white/70">
                            Power
                            {{
                                state === "ongoing"
                                    ? "will resume in"
                        ...
                        <div class="inline md:block font-medium ml-1 md:ml-0">
                            ...
                            <span v-if="state === 'past'">{{
                                formatDate(props.data.to)
                            }}</span>
                            <vue-countdown
                                v-else
                    ...
                    <div
                        class="absolute top-3 right-3 sm:top-4 sm:right-4 md:relative md:top-0 md:right-0"
                    >
                    ...
                    <template #fallback>
                        <div class="md:flex flex-col md:items-end">
                            ...
                            <div
                                class="inline md:block font-medium ml-1 md:ml-0"
                            >
                                <span>Loading...</span>
                            </div>
                        </div>
...
const timeDifference = computed(() => {
    ...
    if (state.value === "ongoing") target = new Date(props.data.to);
    else if (state.value === "upcoming") target = new Date(props.data.from);
    else target = new Date(props.data.to);
